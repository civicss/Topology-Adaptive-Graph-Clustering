...
def initial():
    if args.name == 'reut':
        args.file_name = './output_4/output_4_reut(3)/reut_3.txt'  # 结果文件名字
        args.lr = 1e-4                      # 学习率
        args.k = 3                          # 构建 k 近邻的节点数量
        args.n_clusters = 4                 # 聚类类别数目     
        args.n_input = 2000                 # 输入特征维度
        args.enhance = False                # 是否根据图高阶相似性进行图增强
        args.epoch = 50                     # 训练轮次

        args.ld1 = 1.6                # 损失1: QP KL散度的权重
        args.ld2 = 1e-4                # 损失1: ZP KL散度的权重
        args.ld3 = 1e-4                # 损失1: QZ KL散度的权重
        args.ld4 = 0                # 损失2: 特征重构损失的权重
        args.ld5 = 0                # 损失2: 结构重构损失的权重
        args.ld6 = 0                # 损失3: 相似性损失的权重
        args.ld7 = 1e-4                # 损失4: 提示损失1: 节点与聚类中心均方差
...
def train_AGCN(dataset):
    ...
    # 5.评估聚类结果
    best_acc_QZ, best_nmi_QZ, best_ari_QZ, best_f1_QZ = eva(y, y_pred, 'pae')
...
if __name__ == "__main__":
    ...
    # 添加超参数：根据每个数据集不同需要改的
    parser.add_argument('--name', type=str, default='reut')                 # 数据集名字
    parser.add_argument('--file_name', type=str, default='reut')            # 输出结果文件名字
