...
from evaluation import eva, evao
...
def initial():
    if args.name == 'acm' or args.name == 'acm_for_np':
        args.file_name = './output_4/output_4_acm/acm_4.txt'  # 结果文件名字
        args.k = None                       # 是否构建 KNN 图
        args.n_clusters = 3                 # 聚类类别数目
        args.n_input = 1870                 # 输入特征维度
        args.enhance = True                 # 是否根据图高阶相似性进行图增强
        args.epoch = 60                     # 训练轮次
        args.lr = 1e-3                      # 学习率
        args.ld1 = 0.1                 # 损失1: QP KL散度的权重
        args.ld2 = 0.1                 # 损失1: ZP KL散度的权重
        args.ld3 = 0.1                 # 损失1: QZ KL散度的权重
        args.ld4 = 1                   # 损失2: 特征重构损失的权重
        args.ld5 = 0                   # 损失2: 结构重构损失的权重
        args.ld6 = 0.1                 # 损失3: 相似性损失的权重
        args.ld7 = 1                   # 损失4: 提示损失1: 节点与聚类中心均方差
        args.ld8 = 0                   # 损失4: 提示损失2: 节点与聚类中心交叉熵
        args.ld9 = 1                   # 损失5: 聚类中心正交性约束损失的权重
        args.ld10 = 0                  # 损失6: 高置信度样本伪标签自监督损失的权重

        # args.ld1 = nn.Parameter(torch.tensor(0.1), requires_grad=True)
        # args.ld2 = nn.Parameter(torch.tensor(0.1), requires_grad=True)
        # args.ld3 = nn.Parameter(torch.tensor(0.1), requires_grad=True)
        # args.ld4 = nn.Parameter(torch.tensor(1.0), requires_grad=True)
        # args.ld7 = nn.Parameter(torch.tensor(1.0), requires_grad=True)
        # args.ld9 = nn.Parameter(torch.tensor(1.0), requires_grad=True)
...
def train_AGCN(dataset):
    ...
    list(model.parameters()) +
        [args.ld1, args.ld2, args.ld3, args.ld4, args.ld7, args.ld9, args.ld10]
    """
    optimizer = Adam(
        model.parameters(),
        lr=args.lr,
    ...
    for epoch in range(args.epoch):
        ...
        '''生成目标分布 P'''
        if epoch < 30:
            p = target_distribution(predict.data)          # 计算新的目标分布 p
        else:
        ...
        predict_enhance = target_distribution(predict_enhance.detach())
        res_Z_enhance = predict_enhance.data.cpu().numpy().argmax(1)

        '''考虑增强 QZ: 2次'''
        QZ_enhance = target_distribution(QZ.detach())
        QZ_enhance = target_distribution(QZ_enhance.detach())
        ...
        if epoch % 1 == 0:
            ...
            # '''画图：相似度矩阵'''
            # if epoch % 10 == 0:
            #     plot_similarity_matrix(Z=predict.data.cpu().numpy(),
            #                            cmap='coolwarm', reverse=False, title="Sample Feature Similarity")
        ...
        '''总的损失'''
        # 取消的相似性权重: args.ld6 * (loss_sim_3 + loss_sim_4 + loss_sim_5) + \
...
if __name__ == "__main__":
    ...
    # 添加超参数：根据每个数据集不同需要改的
    parser.add_argument('--name', type=str, default='acm')                  # 数据集名字
    parser.add_argument('--file_name', type=str, default='acm')             # 输出结果文件名字
