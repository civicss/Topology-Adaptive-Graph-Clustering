...
# 代码功能：输入 3 种 motif 对应的邻接矩阵，考虑它们之间的注意力加权
...
def initial():
    if args.name == 'dblp' :
        args.file_name = './output_4/output_4_dblp/dblp_1.txt'  # 结果文件名字
        args.k = None                       # 是否构建 KNN 图
        args.n_clusters = 4                 # 聚类类别数目
        args.n_input = 334                  # 输入特征维度
        args.enhance = False                # 是否根据图高阶相似性进行图增强
        args.epoch = 70                     # 训练轮次
        args.lr = 5e-4                      # 学习率

        args.ld1 = 1                 # 损失1: QP KL散度的权重
        args.ld2 = 1                 # 损失1: ZP KL散度的权重
        args.ld3 = 1                 # 损失1: QZ KL散度的权重
        args.ld4 = 10                # 损失2: 特征重构损失的权重

        args.ld5 = 0                 # 损失2: 结构重构损失的权重
        args.ld6 = 0                 # 损失3: 相似性损失的权重
        args.ld7 = 0                 # 损失4: 提示损失1: 节点与聚类中心均方差
        args.ld8 = 0                 # 损失4: 提示损失2: 节点与聚类中心交叉熵
        args.ld9 = 0.1               # 损失5: 聚类中心正交性约束损失的权重
        args.ld10 = 0.1              # 损失6: 高置信度样本伪标签自监督损失的权重

        # args.ld1 = nn.Parameter(torch.tensor(1.2), requires_grad=True)
        # args.ld2 = nn.Parameter(torch.tensor(1.2), requires_grad=True)
        # args.ld3 = nn.Parameter(torch.tensor(1.2), requires_grad=True)
        # args.ld4 = nn.Parameter(torch.tensor(1.2), requires_grad=True)
        # args.ld5 = nn.Parameter(torch.tensor(0.0), requires_grad=True)
        # args.ld6 = nn.Parameter(torch.tensor(0.0), requires_grad=True)
        # args.ld7 = nn.Parameter(torch.tensor(0.1), requires_grad=True)
        # args.ld8 = nn.Parameter(torch.tensor(0.0), requires_grad=True)
        # args.ld9 = nn.Parameter(torch.tensor(0.0), requires_grad=True)
        # args.ld10 = nn.Parameter(torch.tensor(0.0), requires_grad=True)
...
def reg_loss(C, Z, Z1, device):
    ...
    len1 = len(C)
    loss = torch.zeros(1)           # 初始化损失为一个标量张量，初始值为 0
    loss = torch.FloatTensor(loss)  # 将损失转换为浮点张量
    loss = loss.to(device)          # 将损失移动到指定的设备（CPU 或 GPU）
...
def train_AGCN(dataset):
    ...
    '''list(model.parameters()) +
        [args.ld1, args.ld2, args.ld3, args.ld4, args.ld5, args.ld6, args.ld7, args.ld8, args.ld9, args.ld10],'''
    optimizer = Adam(
        model.parameters(),
        lr=args.lr,
        weight_decay=1e-4)
    ...
    for epoch in range(args.epoch):
        ...
        '''
        x_bar1, _, _, q, h, predict, QZ, pl_loss = model(data, adj_3, adj_4, adj_5, adj)
        ...
        loss_sim_3 = loss_sim_4 = loss_sim_5 = 0
        ...
        # if args.ld6 != 0:
        #     loss_sim_3, edge_num_3 = reg_loss(C_3, q, predict, device=args.device)
        #     loss_sim_3 = loss_sim_3 / edge_num_3
        #     loss_sim_3 = loss_sim_3.item()

        #     loss_sim_4, edge_num_4 = reg_loss(C_4, q, predict, device=args.device)
        #     loss_sim_4 = loss_sim_4 / edge_num_4
        #     loss_sim_4 = loss_sim_4.item()

        #     loss_sim_5, edge_num_5 = reg_loss(C_5, q, predict, device=args.device)
        #     loss_sim_5 = loss_sim_5 / edge_num_5
        #     loss_sim_5 = loss_sim_5.item()
        ...
        loss_prompt_1 = 0
        # 20轮以后，训练到一定程度，聚类结果相对正确再加提示损失
        if epoch > 20:
            if acc_Q > acc_QZ1:
                loss_prompt_1 = F.mse_loss(h, prompt[res_Q])
            else:
                loss_prompt_1 = F.mse_loss(h, prompt[res_QZ_enhance])
        ...
        #       f'args.ld4={args.ld4.item()}, args.ld5={args.ld5}, args.ld6={args.ld6.item()}, '
        #       f'args.ld7={args.ld7.item()}, args.ld8={args.ld8}, args.ld9={args.ld9.item()}, '
        #       f'args.ld10={args.ld10.item()}')
        print(f'KL散度: QP_KL散度={loss_kl_QP}, ZP_KL散度={loss_kl_ZP}, QZ_KL散度={loss_kl_QZ};')
        print(f'重构损失: 特征重构损失={loss_fres_1}; 结构重构损失={loss_sres};')
...
if __name__ == "__main__":
    ...
    # 添加超参数：根据每个数据集不同需要改的
    parser.add_argument('--name', type=str, default='dblp')                 # 数据集名字
    parser.add_argument('--file_name', type=str, default='dblp')            # 输出结果文件名字
