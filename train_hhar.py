...
def initial():
    if args.name == 'hhar':
        args.file_name = './output_4/output_4_hhar(3)/hhar_6.txt'  # 结果文件名字
        args.lr = 1e-3                      # 学习率
        args.k = 3                          # 构建 k 近邻的节点数量
        args.n_clusters = 6                 # 聚类类别数目     
        args.n_input = 561                  # 输入特征维度
        args.enhance = False                # 是否根据图高阶相似性进行图增强
        args.epoch = 120                    # 训练轮次

        args.ld1 = 1                # 损失1: QP KL散度的权重
        args.ld2 = 0.01             # 损失1: ZP KL散度的权重
        args.ld3 = 0                # 损失1: QZ KL散度的权重
...
def train_AGCN(dataset):
    ...
    for epoch in range(args.epoch):
        ...
        '''生成目标分布 P, 分布 P 的预测结果, 考虑增强 P: 2次'''
        p = target_distribution(tmp_q)
        p = target_distribution(p.detach())
...
if __name__ == "__main__":
    ...
    # 添加超参数：根据每个数据集不同需要改的
    parser.add_argument('--name', type=str, default='hhar')                 # 数据集名字
    parser.add_argument('--file_name', type=str, default='hhar')            # 输出结果文件名字
