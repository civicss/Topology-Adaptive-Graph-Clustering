...
def initial():
    if args.name == 'usps':
        args.file_name = './output_4/output_4_usps(3)/usps_1.txt'  # 结果文件名字
        args.lr = 1e-3                      # 学习率
        args.k = 3                          # 构建 k 近邻的节点数量
        args.n_clusters = 10                # 聚类类别数目     
        args.n_input = 256                  # 输入特征维度
        args.enhance = False                # 是否根据图高阶相似性进行图增强
        args.epoch = 100                    # 训练轮次

        args.ld1 = 1             # 损失1: QP KL散度的权重
        args.ld2 = 10            # 损失1: ZP KL散度的权重
        args.ld3 = 1             # 损失1: QZ KL散度的权重
        args.ld4 = 0             # 损失2: 特征重构损失的权重
        ...
        # args.ld1 = nn.Parameter(torch.tensor(1.0), requires_grad=True)
        # args.ld2 = nn.Parameter(torch.tensor(10.0), requires_grad=True)
        # args.ld3 = nn.Parameter(torch.tensor(1.0), requires_grad=True)
        ...
        # args.ld9 = nn.Parameter(torch.tensor(0.1), requires_grad=True)
...
def train_AGCN(dataset):
    ...
    # 5.评估聚类结果
    best_acc_Z, best_nmi_Z, best_ari_Z, best_f1_Z = eva(y, y_pred, 'pae')
    ...
    for epoch in range(args.epoch):
        ...
        '''生成目标分布 P, 分布 P 的预测结果, 考虑增强 P: 1次'''
        # 计算新的目标分布 P(30轮前用 Q，以后用 Z 增强)
        ...
        print(f'重构损失: 特征重构损失={loss_fres_1};')
        # print(f'相似性损失={loss_sim_3}、{loss_sim_4}、{loss_sim_5};')
        # print(f'提示损失1(节点与聚类中心均方差)={loss_prompt_1};')
    ...
    with open(args.file_name, 'a') as file:
        ...
        file.write(f"Z Result: acc={best_acc_Z:.4f}, nmi={best_nmi_Z:.4f}, ari={best_ari_Z:.4f}, f1={best_f1_Z:.4f}\n")
        file.write(f"Z增强 Result: acc={best_acc_Z1:.4f}, nmi={best_nmi_Z1:.4f}, ari={best_ari_Z1:.4f}, f1={best_f1_Z1:.4f}\n")
...
if __name__ == "__main__":
    ...
    # 添加超参数：根据每个数据集不同需要改的
    parser.add_argument('--name', type=str, default='usps')                 # 数据集名字
    parser.add_argument('--file_name', type=str, default='usps')            # 输出结果文件名字
